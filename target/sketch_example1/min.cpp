...
static void send_ack(struct min_context *self)
{
    ...
    if (ON_WIRE_SIZE(0) <= min_tx_space(self->port)) {
        on_wire_bytes(self, ACK, self->transport_fifo.rn, &self->transport_fifo.rn, 0, ACK, 1U);
    ...
    default:
        if (id_control & 0x80U) {
            ...
            if (seq == self->transport_fifo.rn) {
                ...
                min_application_handler(self, id_control & (uint8_t)0x3fU, payload, payload_len, self->port);
            } else {
...
void min_application_handler(struct min_context *self, uint8_t min_id, uint8_t const *min_payload, uint8_t len_payload, uint8_t port)
{
  ...
  min_debug_print("MIN frame with ID %d", min_id);
...
static void rx_byte(struct min_context *self, uint8_t byte) // receive byte on the wire
{
    ...
    if (self->rx_header_bytes_seen == 2) { // HEADER_BYTE = 0xAAU
        self->rx_header_bytes_seen = 0;
        if (byte == HEADER_BYTE) {
            self->rx_frame_state = RECEIVING_ID_CONTROL; // if the third byte in a row is a header_byte then set rx_frame_state to receive
            return; // exits function rx_byte immediately and returns to where it was called
        }
        if (byte == STUFF_BYTE) { // else the third byte was not header, so discard and start searching for another SOF
    ...
    if (byte == HEADER_BYTE) { // if header_byte seen
        self->rx_header_bytes_seen++; // incremement rx_header_bytes_seen
    } else { // else
        self->rx_header_bytes_seen = 0; // set it to 0
    }

    switch (self->rx_frame_state) { // activate switch case with 10 cases plus default
    case SEARCHING_FOR_SOF: // not associated with anything in enum, but think of it as case 0.
        break; // this case indicates that nothing has been received
    case RECEIVING_ID_CONTROL: // case 1: received three header bytes in a row. Get ready to read the frame id
        self->rx_frame_id_control = byte;
        self->rx_frame_payload_bytes = 0;
        crc32_init_context(&self->rx_checksum); // initialize checksum to be 0xffffffffU
        crc32_step(&self->rx_checksum, byte); // calculate checksum 
        if (byte & 0x80U) { // if MSb of byte = 1
#ifdef TRANSPORT_PROTOCOL // and if TRANSPORT_PROTOCOL is defined
            self->rx_frame_state = RECEIVING_SEQ; // set rx_frame_state to receiving_seq which is case
#else
        ...
        break;
    case RECEIVING_SEQ: // case 2: see line 487. if byte = 0x80 then prepare to receive a sequence?
        self->rx_frame_seq = byte; // this is the first frame in the sequence
        crc32_step(&self->rx_checksum, byte); // store byte in rx_checksum
        self->rx_frame_state = RECEIVING_LENGTH; // set rx_frame_state to receive the length of sequence which is case 3.
        break;
    case RECEIVING_LENGTH: // case 3: see line 499. 
...
void min_send_frame(struct min_context *self, uint8_t min_id, uint8_t const *payload, uint8_t payload_len)
{
    ...
    }                 // (struct, id control, seq, payload base, payload offset, payload mask, payload len)
}
